...
export function vfs_read_file(file_id) {
  ...
  return op_queue_push(
    ...
    (b) => {
      return b.readBytes(b.remaining()).toArrayBuffer();
    },
  );
}
...
function write_meta(bb, meta) {
  ...
  if (bytes.length > 90) {
    throw new Error(
      i18n.t("properties.remarktoolong") +
        bytes.length +
        i18n.t("properties.remarktoolongend"),
    );
  }
