...
def read_v3_amiibo_from_csv():
    csv_file = get_prorject_directory() + "/data/amiidb_v3.csv"
    if not os.path.exists(csv_file):
        return list()
    special_amiibos = list()
    with open(csv_file, "r", encoding="utf8") as f:
        for r in csv.reader(f):
            if len(r) > 0 and len(r[0]) == 16:
                amiibo = Amiibo()
                amiibo.id = r[0]
                special_amiibos.append(amiibo)
    return special_amiibos


def gen_v3_amiibo_c_file(special_amiibos):
    c_file = get_prorject_directory() + "/application/src/amiidb/db_v3.c"
    with open(c_file, "w+", newline="\n", encoding="utf8") as f:
        f.write('/* This file is auto-generated by amiibo_db_gen.py. Do not edit directly. */\n')  
        f.write('#include "db_header.h"\n')
        f.write('const db_v3_t v3_list[] = {\n')
        for amiibo in special_amiibos:
            f.write('{0x%s, 0x%s}, \n' % 
...
# Process special amiibo from amiidb_v3.csv
v3_amiibos = read_v3_amiibo_from_csv()
gen_v3_amiibo_c_file(v3_amiibos)
