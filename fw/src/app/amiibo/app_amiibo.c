...
static void app_amiibo_list_view_on_selected(mui_list_view_t *p_list_view, mui_list_item_t *p_item) {
    ...
    mui_view_dispatcher_switch_to_view(p_app_handle->p_view_dispatcher, AMIIBO_VIEW_ID_DETAIL);
}

static void app_amiibo_view_on_close(amiibo_view_t *p_view) {
    app_amiibo_t *p_app_handle = p_view->user_data;
    mui_view_dispatcher_switch_to_view(p_app_handle->p_view_dispatcher, AMIIBO_VIEW_ID_LIST);
}
