...
void amiibo_scene_storage_list_menu_on_enter(void *user_data) {
    ...
    if (res == VFS_OK) {
        ...
        #define mount_str "===已挂载[LFS]==="
        #else
        #define mount_str "===已挂载[FFS]==="
        #endif
        mui_list_view_add_item(app->p_list_view, 0xe1ca,
                               stat.avaliable ? mount_str : "=====未挂载=====", (void *)-1);
        if (stat.avaliable) {
    ...
    mui_list_view_add_item(app->p_list_view, ICON_HOME, get_message(MESSAGE_ID_BACK_TO_MAIN_MENU), (void *)STORAGE_LIST_MENU_BACK_MAIN_MENU);
