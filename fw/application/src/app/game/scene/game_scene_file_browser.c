...
static void game_scene_file_browser_on_selected(mui_list_view_event_t event, mui_list_view_t *p_list_view,
    ...
    if (event == MUI_LIST_VIEW_EVENT_SELECTED) {
        if (p_item->icon == ICON_FILE) {
            string_set_str(app->selected_file, "/game/");
...
static int game_scene_file_browser_list_item_cmp(const mui_list_item_t *p_item_a, const mui_list_item_t *p_item_b) {
...
static void game_scene_file_browser_reload_folders(app_game_t *app) {
    ...
    int32_t res = p_vfs_driver->open_dir("/game", &dir);
    uint32_t file_cnt = 0;
    if (res == VFS_OK) {
    ...
    //mui_list_view_sort(app->p_list_view, game_scene_file_browser_list_item_cmp);
}

void game_scene_file_browser_on_enter(void *user_data) {
    app_game_t *app = user_data;
    game_scene_file_browser_reload_folders(app);
    mui_list_view_set_selected_cb(app->p_list_view, game_scene_file_browser_on_selected);
    mui_view_dispatcher_switch_to_view(app->p_view_dispatcher, GAME_VIEW_ID_LIST);
}

void game_scene_file_browser_on_exit(void *user_data) { app_game_t *app = user_data; }