...
void settings_scene_version_on_enter(void *user_data) {
    ...
    char txt[32];
    sprintf(txt, get_message(MESSAGE_ID_VERSION_ARG_STRING), version_get_version(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1c7, txt, (void *)0xFF);

    sprintf(txt, get_message(MESSAGE_ID_BUILD_DATE_ARG_STRING), version_get_builddate(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1c8, txt, (void *)0xFF);

    sprintf(txt, get_message(MESSAGE_ID_GIT_HASH_ARG_STRING), version_get_githash(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1c9, txt, (void *)0xFF);

    sprintf(txt, get_message(MESSAGE_ID_BRANCH_ARG_STRING), version_get_gitbranch(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1ca, txt, (void *)0xFF);

    sprintf(txt, get_message(MESSAGE_ID_TARGET_ARG_INT), version_get_target(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1cb, txt, (void *)0xFF);

    sprintf(txt, get_message(MESSAGE_ID_DIRTY_ARG_INT), version_get_dirty_flag(NULL));
    mui_list_view_add_item(app->p_list_view, 0xe1cc, txt, (void *)0xFF);

    mui_list_view_add_item(app->p_list_view, 0xe1cd, get_message(MESSAGE_ID_BACK), (void *)0);
